...
static int test_search(RadixMap *map)
{
	...
	for (i = map->end / 2; i < map->end; i++)
	{
		...
		check(found->data.key == d->data.key, "Got the wrong result: %p:%u looking for %d at %u", found, found->data.key, d->data.key, i);
	}
...
static char *test_operations()
{
	...
	mu_assert(test_search(map), "Failed to the search test.");
	...
	while(map->end >0) {
		...
		mu_assert(check_order(map), "RadixMap didn't sorted after delete.");
	}
